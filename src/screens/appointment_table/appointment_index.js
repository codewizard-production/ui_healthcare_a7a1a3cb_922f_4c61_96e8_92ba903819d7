...
const columns = [
    { headerName: "AppointmentID", field: "AppointmentID", flex: 1 },
    { headerName: "Date", field: "Date", flex: 1 },
    { headerName: "Doctor", field: "DoctorName", flex: 1 },
    { headerName: "Doctor", field: "Specialization", flex: 1 },
];
...
const Component = (props) => {
    ...
    const FetchResults = async () => {
        ...
        await Api.GetAppointmentsMulti(query, "Doctor")
            .then(async (res) => {
                if (res.status) {
                    _rows = res.values || [];
                    if (_rows.length > 0) {
                        _rows.forEach(x => {
                            ...
                            x.DoctorName = x.Doctor?.DoctorName || 'NA';


                            x.Specialization = x.Doctor?.Specialization || 'NA';
    ...
    const OnActionClicked = (id, type) => {
        let _route;
        if (type === 'edit') _route = `/Appointments/edit/${id}`;
                    ...
                    <Stack direction="row">
                        <Grid container sx={{ justifyContent: 'flex-end' }}>
                            ...
                            <IconButton
                                ...
                                onClick={() => NavigateTo("/Appointments/create")}
                            >
                                <AddBoxIcon />
                            </IconButton>
                        </Grid>
