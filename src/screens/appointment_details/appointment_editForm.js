...
const Component = (props) => {
    ...
    const OnSubmit = async () => {
        let rslt, data, changes = [], appointmentId, appointment, numfields, mapItem;

        appointment = row['appointment'];
        appointmentId = row['appointment'].find((x) => x.type === 'keyid').value;
                ...
                if (changes.length > 0) {
                    ...
                    if (tmp.length > 0) {
                        ...
                        if (rslt.status) {
                            ...
                            if (Helper.IsNullValue(mapItem.navpropname)) appointmentId = rslt.id;
                            
                            const mapPropKey = appointment.find(x => x.uicomponent === mapItem.uicomponent).key;
                            data = [
                                { key: "AppointmentID", value: parseInt(appointmentId) },
                                { key: mapPropKey, value: parseInt(rslt.id) }
                            ];
                            rslt = await Support.AddOrUpdateAppointment(data, dropDownOptions);
        ...
        // Update root entity
        mapItem = MapItems.find(x => x.uicomponent === 'appointment');
        changes = TrackChanges(mapItem.uicomponent);
        if (changes.length > 0) {
            let tmp = changes.filter((x) => mapItem.exclude.indexOf(x) === -1);
            if (tmp.length > 0) {
